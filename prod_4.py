...
def modelprocess():
   # Training_columns = request.form.getlist('Training_Columns')
    ...
    fig=px.bar(feat_imp, x='Importance', y='Features',width=800, height=500)
   ...
   #filename='C:\Users\Sravan\Desktop\Product_3\templates\img2.html'
    ...
    return render_template('model_target.html',fig=fig,columns=df.columns)
...
def predict_view():  
    ...
    fig = px.histogram(df,x=Target,color=Target,width=500, height=400)
    ...
    return render_template("predict_view.html",df1=df.to_html(classes='table table-striped table-hover')) 
...
def forecast_modelprocess():
  ...
  #  model = request.form.get('Model')
   ...
   #filename='C:\Users\Sravan\Desktop\Product_3\templates\img2.html'
    ...
    return render_template('forecast_model1.html',fig=fig,columns=df.columns)
...
def forecast_view():
        ...
        plotly_graphs.forecast_plot(vs_df)
        
        vs_df= vs_df.drop(['date'],axis=1)
          
        return render_template("forecast_view2.html",vs_df=vs_df.to_html(classes='table table-striped table-hover'))
...
def clustering_modelprocess():
    ...
    plotly_graphs.cluster_importance(df,variables_for_segmenting,clusters)
...
def clustering_view():  
    file2 = request.files['file'] 
   # df1=df.head(100)   
    plotly_graphs.scatter(df)
    return render_template("clustering_view.html",df1=df.to_html(classes='table table-striped table-hover')) 
...
def vs1():
    ...
    if request.form.get("Insurance_Claim1"):
        ...
        vs_df=pd.read_csv("data/appointment.csv")
        vs_model = pickle.load(open('data/appointment.pkl', 'rb'))
        vs_title="Appointment No show"
        vs_df = vs_df.fillna(vs_df.median())
        vs_x = vs_df.drop(["days"],axis=1)
        vs_y = vs_df["days"] 
        vs_Target="days"
        method="prediction"
    
    elif request.form.get("Insurance_Claim10"):
        vs_df=pd.read_csv("data/predictive_maintenance.csv")
        vs_model = pickle.load(open('data/predictive_maintenance.pkl', 'rb'))
        vs_title="Predictive Maintinence"
        vs_df = vs_df.fillna(vs_df.median())
        vs_x = vs_df.drop(["mcc"],axis=1)
        vs_y = vs_df["mcc"] 
        vs_Target="mcc"
        method="prediction"
        ...
        vs_title="Gas Demand"
        method="forecasting"
        return render_template('gas_forecast.html',vs_title=vs_title)
    
    elif request.form.get("Insurance_Claim41"):
        vs_model = pickle.load(open('data/sales_forecast.pkl', 'rb'))
        vs_title="Forex Close Price"
        method="forecasting"
        return render_template('forex_forecast.html',vs_title=vs_title)
    
    elif request.form.get("Insurance_Claim45"):
        vs_model = pickle.load(open('data/sales_forecast.pkl', 'rb'))
        vs_title="Web Traffic"
        method="forecasting"
        return render_template('webtraffic_forecast.html',vs_title=vs_title)
  ...
  #  vs_sample.to_csv('static/'+sample_uri)
    ...
    return render_template('vs2_1.html',vs_title=vs_title,x_columns=vs_x.columns,df=vs_df,sample_uri=sample_uri)
...
def order_quantity():    
    ...
    if vs_title=="Demand Forecast":        
        ...
        return render_template('order_quantity.html',vs_title=vs_title,vs_df=vs_df.to_html(classes='table table-striped table-hover'))
        ...
        return render_template('sales_forecast.html',vs_title=vs_title,vs_df=vs_df.to_html(classes='table table-striped table-hover'))
...
def vs2_2():
    ...
    return render_template('vs2_1.html',vs_title=vs_title,x_columns=vs_x.columns,df=vs_df, prediction_text='The Prediction value is {}'.format(output))
...
def vs2_3():
    ...
    if vs_title=="Insurance Claim Prediction":  
        ...
        df3['insuranceclaim'].replace([0,1],['Rejected','Accepted'],inplace=True)
        ...
        plotly_graphs.Bar_Chart(vs_dfn,Bxvar,Byvar,Bcolor,Btitle,Bxaxes,Byaxes)
        ...
        df3['Outcome'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['Outcome']
       ...
       # vs_dfn['cluster'] = model.labels_        
        ...
        plotly_graphs.Scatter_3D(vs_dfn,S3xvar,S3yvar,S3zvar,S3color,S3title)
        ...
        plotly_graphs.Hist_Pred(vs_dfn,Hxvar,Hcolor,Htitle,Hxaxis,Hyaxis)
        ...
        df3['class'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['class']
        ...
        df3 = vs_dfn.groupby(["days"]).count().reset_index()
        df3['days'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['days']
        Byvar=vs_dfn.groupby(["days"]).size()
        Bcolor=df3['days']
        Btitle='Appointment No show'
        Bxaxes='Days'
        Byaxes='Count of days'
        plotly_graphs.Bar_Chart(vs_dfn,Bxvar,Byvar,Bcolor,Btitle,Bxaxes,Byaxes)
        
        method="classification"
    
    elif vs_title=="Predictive Maintinence":
        
        df3 = vs_dfn.groupby(["mcc"]).count().reset_index()
        #df3['mcc'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['mcc']
        Byvar=vs_dfn.groupby(["mcc"]).size()
        Bcolor=df3['mcc']
        Btitle='Predictive Maintinence'
        Bxaxes='Maintinence'
        Byaxes='Count of persons'
        plotly_graphs.Bar_Chart(vs_dfn,Bxvar,Byvar,Bcolor,Btitle,Bxaxes,Byaxes)
        ...
        df3['readmitted'].replace([0,1],['No','Yes'],inplace=True)
        ...
        df3['Deposit'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['Deposit']
        ...
        df3['isFradulent'].replace([0,1],['No Fraud','Fraud'],inplace=True)
        ...
        plotly_graphs.Scatter_2D(vs_dfn,Sxvar,Syvar,Scolor,Stitle,Sxaxes,Syaxes)
        ...
        df3['attack_type'].replace([0,1],['No','Yes'],inplace=True)
    ...
    elif vs_title=="Telecom_Churn":
        
        df3 = vs_dfn.groupby(["Churn"]).count().reset_index()
        df3['Churn'].replace([0,1],['customer continuation','customer cancellation'],inplace=True)
        Bxvar=df3['Churn']
        ...
        df3['Attrition'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['Attrition']
        ...
        df3['intime'].replace([0,1],['Delivery Pass','Delivery Fail'],inplace=True)
        Bxvar=df3['intime']
        ...
        Sxvar=vs_dfn['Solar energy']
        Syvar=vs_dfn['Cloud coverage']
        Scolor=vs_dfn['Temperature']
        Stitle='Solar Energy Prediction'
        Sxaxes='Solar energy'
        Syaxes='Cloud coverage'
        plotly_graphs.Scatter_2D(vs_dfn,Sxvar,Syvar,Scolor,Stitle,Sxaxes,Syaxes)
        ...
        df3['Sale'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['Sale']
        ...
        df3['Accepted'].replace([0,1],['Rejected','Accepted'],inplace=True)
        Bxvar=df3['Accepted']
        ...
        df3['Other_Faults'].replace([0,1],['Defect','No Defect'],inplace=True)
        ...
        df3['isFraud'].replace([0,1],['No','Yes'],inplace=True)
        Bxvar=df3['isFraud']
    ...
    return render_template("vs2_2.html",vs_title=vs_title,vs_df=vs_dfn.to_html(classes='table table-striped table-hover'),method=method) 
